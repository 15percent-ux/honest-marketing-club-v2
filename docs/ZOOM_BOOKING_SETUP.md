...
5. **ダブルブッキング防止** — Googleカレンダーの既存予定（手動で入れた予定も含む）と重なる枠は予約画面に表示されません
...
- バックエンド: Netlify Functions（`netlify/functions/`）
  ...
  - `send-reminders.mts` — 15分ごとの定期実行でリマインドメールを送信
...
4. テスト予約の削除は、Googleカレンダーの予定とZoomのミーティングをそれぞれ削除すればOKです
